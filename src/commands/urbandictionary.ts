...
export async function handleComponentInteraction(interaction: ComponentInteraction) {
  ...
  if (definitions === undefined)
    return interaction.createFollowup({ flags: MessageFlags.EPHEMERAL, content: 'failed to get pagination context, try doing a new `/urbandictionary` and interacting with that one' })
